...
with torch.no_grad():
    for i in range(10):
        ...
        plt.savefig(f"{train_config['output_dir']}/original_{dataset.__class__.__name__}_ex{i}.png")
        
        numbers = model.generate(single_image,
                    num_examples=5)
        
        for j, number in enumerate(numbers):
            number = number.permute(1, 2, 0).detach().numpy()
            plt.figure()
            plt.imshow(number)
            plt.savefig(f"{train_config['output_dir']}/generated_ex{i}_{j}.png")
